...
public class FalconMap {
	...
	public void initMap(GameController gc) {
		...
		this.map = new MapNode[height][width];
		this.karboniteDeposits = new ArrayList<MapNode>();
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				...
				MapNode node = new MapNode(j, i, karbonite, tag, m.isPassableTerrainAt(tmp) == 1);
				map[i][j] = node;
	...
	public MapLocation searchForKarbonite(int centerX, int centerY) {
		...
		int maxRadius = Math.max(Math.max(this.width - 1 - centerX, centerX), Math.max(this.height - 1 - centerY, centerY));
